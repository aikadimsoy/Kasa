...
(function() {
    ...
    rail.style.background = `var(--kasa-n900)`;
    rail.style.zIndex = '2147483646';
    rail.style.display = 'flex';
    rail.style.flexDirection = 'column';
    rail.style.alignItems = 'center';
    rail.style.gap = '8px';
    rail.style.paddingTop = '12px';
    rail.style.boxShadow = `var(--kasa-e2)`;

    const hamburgerIcon = document.createElement('svg');
    hamburgerIcon.innerHTML = `<path d="M4 6h16a2 2 0 0 0 0-4H4a2 2 0 0 0 0 4zm0 6h16a2 2 0 0 0 0-4H4a2 2 0 0 0 0 4zm0 6h16a2 2 0 0 0 0-4H4a2 2 0 0 0 0 4z" fill="none" stroke="currentColor" stroke-width="1.75"/>`;
    hamburgerIcon.style.width = '20px';
    hamburgerIcon.style.height = '20px';
    hamburgerIcon.style.cursor = 'pointer';
    rail.appendChild(hamburgerIcon);

    const araclarIcon = document.createElement('svg');
    araclarIcon.innerHTML = `<path d="M8 10v6a2 2 0 0 0 2 2h4a2 2 0 0 0 2-2v-6m-4 9H7a3 3 0 0 1-3-3V8a3 3 0 0 1 3-3h5l3 3v7a3 3 0 0 1-3 3z" fill="none" stroke="currentColor" stroke-width="1.75"/>`;
    araclarIcon.style.width = '20px';
    araclarIcon.style.height = '20px';
    araclarIcon.style.cursor = 'pointer';
    rail.appendChild(araclarIcon);

    const kullaniciIcon = document.createElement('svg');
    kullaniciIcon.innerHTML = `<path d="M16 18H8a4 4 0 0 1-4-4V5a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v9a4 4 0 0 1-4 4z" fill="none" stroke="currentColor" stroke-width="1.75"/>`;
    kullaniciIcon.style.width = '20px';
    kullaniciIcon.style.height = '20px';
    kullaniciIcon.style.cursor = 'pointer';
    rail.appendChild(kullaniciIcon);

    const gizlilikIcon = document.createElement('svg');
    gizlilikIcon.innerHTML = `<path d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm0 8c4.5 0 9-3 9-9s-3.5-9-9-9S3 7.5 3 12s4.5 9 9 9z" fill="none" stroke="currentColor" stroke-width="1.75"/>`;
    gizlilikIcon.style.width = '20px';
    gizlilikIcon.style.height = '20px';
    gizlilikIcon.style.cursor = 'pointer';
    rail.appendChild(gizlilikIcon);
    ...
    panel.style.background = `var(--kasa-n950)`;
    panel.style.zIndex = '2147483646';
    panel.style.boxShadow = `var(--kasa-e2)`;
    panel.style.overflowY = 'auto';
    panel.style.padding = '16px';
    panel.style.color = `var(--kasa-n100)`;
    panel.style.fontFamily = 'KasaUI, sans-serif';
    panel.style.fontSize = '14px';
    panel.style.transform = 'translateX(-320px)';
    panel.style.transition = `transform var(--kasa-t-std) var(--kasa-ease)`;

    const sections = {
        araclar: `<h2>Araclar</h2><label><input type="checkbox" id="tool_screenshot"> Ekran goruntusu</label><br><label><input type="checkbox" id="tool_savetext"> Sayfa metnini kaydet</label><br><label><input type="checkbox" id="tool_cookies"> Cerez goruntule</label><br><label><input type="checkbox" id="tool_vault"> Vault'a gonder</label>`,
        kullanici: `<h2>Kullanici Ayarlari</h2><label for="user_alias">Takma ad: <input type="text" id="user_alias"></label><br><label for="search_engine">Arama motoru:<select id="search_engine"><option value="duckduckgo">DuckDuckGo</option><option value="startpage">Startpage</option><option value="brave">Brave</option></select></label><br><label for="theme">Tema:<select id="theme"><option value="dark">Koyu</option><option value="system">Sistem</option></select></label>`,
        gizlilik: `<h2>Gizlilik Seviyesi</h2><div class="level-card" data-value="off"><strong>Kapali</strong>: "Hicbir sahte veri yok. Gercek parmak izin gonderilir."</div><div class="level-card" data-value="standard"><strong>Standart</strong>: "Konum, dil, saat dilimi, ekran sahte. Canvas/WebGL dokunulmaz."</div><div class="level-card" data-value="strict"><strong>Siki</strong>: "Standart + Canvas/WebGL parmak izi zehirleme + tracker cerez zehirleme."</div><div class="level-card" data-value="paranoid"><strong>Paranoyak</strong>: "Siki + bilinen tracker istekleri engellenir (deneysel)."`
    };

    let currentSection = 'araclar';
    ...
    rail.addEventListener('click', (e) => {
        ...
        if (target === hamburgerIcon) {
            document.body.style.marginLeft = panel.style.transform === 'translateX(0)' ? '0' : '56px';
            panel.style.transform = panel.style.transform === 'translateX(0)' ? 'translateX(-320px)' : 'translateX(0)';
        } else if (target === araclarIcon) {
            switchSection('araclar');
        } else if (target === kullaniciIcon) {
            switchSection('kullanici');
        } else if (target === gizlilikIcon) {
            switchSection('gizlilik');
        }
    });

    function updateUI() {
        const tools = ['_kasa_tool_screenshot', '_kasa_tool_savetext', '_kasa_tool_cookies', '_kasa_tool_vault'];
        tools.forEach(tool => {
            const checkbox = panel.querySelector(`#${tool}`);
            if (checkbox) localStorage.setItem(tool, checkbox.checked);
        });

        const userAlias = panel.querySelector('#user_alias');
        if (userAlias && userAlias.value !== localStorage.getItem('_kasa_user_alias')) {
            localStorage.setItem('_kasa_user_alias', userAlias.value);
        }

        const searchEngineSelect = panel.querySelector('#search_engine');
        if (searchEngineSelect && searchEngineSelect.value !== localStorage.getItem('_kasa_user_search')) {
            localStorage.setItem('_kasa_user_search', searchEngineSelect.value);
        }

        const themeSelect = panel.querySelector('#theme');
        if (themeSelect && themeSelect.value !== localStorage.getItem('_kasa_user_theme')) {
            localStorage.setItem('_kasa_user_theme', themeSelect.value);
        }

        const privacyLevels = ['off', 'standard', 'strict', 'paranoid'];
        const levelCards = panel.querySelectorAll('.level-card');
        levelCards.forEach(card => {
            if (card.getAttribute('data-value') === localStorage.getItem('_kasa_privacy_level')) {
                card.style.border = '2px solid var(--kasa-primary)';
            } else {
                card.style.border = '1px solid var(--kasa-n700)';
            }
        });
    }

    const privacyLevels = ['off', 'standard', 'strict', 'paranoid'];
    panel.innerHTML = `<h2>Gizlilik Seviyesi</h2>${privacyLevels.map(level => `<div class="level-card" data-value="${level}"><strong>${level.charAt(0).toUpperCase() + level.slice(1)}</strong>: "${getPrivacyDescription(level)}"</div>`).join('')}`;

    function getPrivacyDescription(level) {
        switch (level) {
            case 'off': return "Hicbir sahte veri yok. Gercek parmak izin gonderilir.";
            case 'standard': return "Konum, dil, saat dilimi, ekran sahte. Canvas/WebGL dokunulmaz.";
            case 'strict': return "Standart + Canvas/WebGL parmak izi zehirleme + tracker cerez zehirleme.";
            case 'paranoid': return "Siki + bilinen tracker istekleri engellenir (deneysel).";
            default: return "";
        }
    }
