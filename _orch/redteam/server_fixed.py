...
class ExecuteToolRequest(BaseModel):
    ...
    agent_id: str = Field(..., description="Araçları çalıştıran ajanın kimliği.")
...
async def execute_tool(
    ...
    for tool_call in request.tool_calls:
        ...
        if tool_name not in PUBLIC_TOOLS:
            raise HTTPException(status_code=404, detail=f"Araç bulunamadı: '{tool_name}'")
        ...
        try:
            ...
            raise HTTPException(status_code=422, detail=f"'{tool_name}' aracı için geçersiz parametreler: {e}")
