...
class VaultTools:
    ...
    def profile_write(self, key: str, value: any, provenance: list) -> dict:
        ...
        cursor.execute(
            ...
            (key, json.dumps(value), json.dumps(provenance), now, now, key)
        )
        conn.commit()
    ...
    def audit_read(self, start_index: int = 0, count: int = 100) -> dict:
        ...
        records = [
            {"id": r[0], "timestamp": r[1], "agent_id": r[2],
             "action": r[3], "details": json.loads(r[4] or '{}'), "hash": r[5]}
            for r in rows
        ]
        result = {"status": "success", "count": len(records), "records": records}

        self.audit_chain.record(self.agent_id, action, {**details, "result": "success", "count": len(records)})
        return result

    def event_ingest(self, source: str, event_type: str, content: dict, ttl_days: int = 30) -> dict:
        """
        Belirli bir kaynaktan gelen ve belirtilen türde olayları Kasa'ya ekler.

        Args:
            source: Olayın kaynağı (örn: 'API', 'UserInput').
            event_type: Olayın türü (örn: 'login', 'data_access').
            content: Olayın içeriği (JSON-serileştirilebilir bir sözlük).
            ttl_days: Olayın geçerli kalma süresi (gün cinsinden, varsayılan 30 gün).

        Returns:
            İşlem sonucunu ve eklenen olayın ID'sini içeren bir sözlük.
        """
        action = "event_ingest"
        details = {"source": source, "type": event_type, "ttl_days": ttl_days}
        ...
        if not isinstance(event_type, str) or not event_type:
            raise ValueError("Invalid 'event_type': must be a non-empty string.")
        if not isinstance(content, dict):
            raise ValueError("Invalid 'content': must be a dictionary.")
        if not isinstance(ttl_days, int) or ttl_days < 1 or ttl_days > 365:
            raise ValueError("Invalid 'ttl_days': must be an integer between 1 and 365.")
        ...
        cursor.execute(
            ...
            (timestamp, self.agent_id, source, event_type, json.dumps(content), timestamp + ttl_days * 86400)
        )
        event_id = cursor.lastrowid
        conn.commit()

        # Audit the event ingestion
        self.audit_chain.record(self.agent_id, action, {**details})
