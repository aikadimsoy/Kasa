...
def server_client(tmp_vault):
    ...
    importlib.reload(importlib.import_module("src.mcp_server.server"))
    srv = importlib.import_module("src.mcp_server.server")
    token = srv._BEARER_TOKEN
    client = TestClient(srv.app, raise_server_exceptions=False)
    yield type('Obj', (), {'client': client, 'token': token, 'headers': {"Authorization": f"Bearer {token}"}})()
    del os.environ["KASA_VAULT_PATH"]
